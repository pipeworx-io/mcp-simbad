...
`tools/list` at `https://gateway.pipeworx.io/simbad/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/simbad_object \
  -H 'Content-Type: application/json' \
  -d '{"identifier":"M31"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/simbad_object`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
